...
function renderVoiceSearchResults() {
  ...
  box.querySelectorAll("[data-voice-save]").forEach(btn => {
    btn.addEventListener("click", e => {
      ...
      const v = voices.find(x => String(x.id) === String(id));
                          ...
                          // Premium Save = same Premium dialog as Select
                          if (badge.className === "premium") {
                            sessionStorage.setItem(
                              "speakora_premium_save_voice",
                              String(id)
                            );
  ...
  const finalizeVoiceSelection = (id, preserveSearch = false) => {
  if (!id) return false;
  ...
  // Normal voice selection clears search/filter state.
  // Saved voice selection keeps the current search/filter state.
  if (!preserveSearch) {
    renderVoiceSearchResults();
    ...
    if (input) {
      ...
      document.getElementById("voiceLanguageFilter");
      document.getElementById("voiceGenderSelect");
      document.getElementById("voiceCategoryFilter");
    ...
    const resultsBox =
      document.getElementById("voiceSearchResultsBox");
    const resultCount =
      document.getElementById("voiceResultCount");
...
let pendingPremiumVoiceId = "";
  ...
  const openPremiumVoiceModal = id => {
    ...
    const v =
      voices.find(x => String(x.id) === String(id)) ||
      getRecentVoices().find(x => String(x.id) === String(id));
  ...
  if (premiumAd) {
    premiumAd.onclick = () => {
      ...
      sessionStorage.setItem(
        "speakora_pending_premium_voice",
        id
      );

      sessionStorage.setItem(
        "speakora_premium_return",
        "1"
      );

      window.location.href = SPEAKORA_SMARTLINK;
    };
  }
  ...
  window.addEventListener("pageshow", () => {
    ...
    const returned =
      sessionStorage.getItem("speakora_premium_return");

    const fromSaved =
      sessionStorage.getItem(
        "speakora_premium_from_saved"
      ) === "1";

    const savedPremiumId =
      sessionStorage.getItem(
        "speakora_premium_save_voice"
      );
    ...
    if (pendingId && returned === "1") {
      ...
      setTimeout(() => {
        showSpeakOraThankYou(() => {
          ...
          if (
            savedPremiumId &&
            String(savedPremiumId) === String(pendingId)
          ) {
            rememberRecentVoice(pendingId);
            renderRecentVoices();
          }

          finalizeVoiceSelection(
            pendingId,
            fromSaved
          );
        });
...
function showSpeakOraThankYou(onClose) {
  ...
  if (!modal) {
    ...
    modal.id = "speakoraThankYouModal";
    ...
    const style = document.createElement("style");
    style.id = "speakoraThankYouStyle";
    style.textContent = `
      #speakoraThankYouModal {
        position: fixed;
        inset: 0;
        z-index: 1000000;
        display: flex;
  ...
  const closeBtn = document.getElementById("speakoraThankYouClose");
...
function renderRecentVoices() {
  ...
  box.querySelectorAll("[data-recent-select]").forEach(btn => {
    btn.onclick = e => {
      ...
      const liveVoice =
        voices.find(v => String(v.id) === String(saved.id)) || null;
      ...
      const savedIsPremium =
        ...
        !!(
          liveVoice &&
          getVoiceProfessionalBadge(
            getVoiceProfessionalScore(liveVoice)
          ).className === "premium"
        );

      box.querySelectorAll(".recent-voice-item")
        .forEach(item =>
          item.classList.remove("recent-voice-selected")
        );
      ...
      /*
       * SAVED PREMIUM VOICE:
       * Use EXACTLY the same Premium dialog as normal voice selection.
       */
      if (savedIsPremium) {
        if (liveVoice) {
          openPremiumVoiceModal(saved.id);
        }
        return;
      }
      ...
      /*
       * SAVED NORMAL VOICE:
       * Select normally, but clear the search/results first.
       */
      const input = document.getElementById("voiceSearch");
      ...
      // Selecting a saved voice clears the active search/results.
    ...
    finalizeVoiceSelection(saved.id);
    };
  });
}
  ...
  if (index > -1) {
    ...
    const isPremium =
      !!(
        savedVoice &&
        getVoiceProfessionalBadge(
          getVoiceProfessionalScore(savedVoice)
        ).className === "premium"
      );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (supportBtn) {
    supportBtn.addEventListener("click", () => {
      sessionStorage.setItem("speakora_pending_generation", "1");
  ...
  window.addEventListener("pageshow", () => {
    const pending = sessionStorage.getItem("speakora_pending_generation");
